...
function build(c::Connection, cm::ComponentModifier, cell::Cell{:R},
    ...
    style!(sideb, "background-color" => "darkblue")
...
function evaluate(c::Connection, cm2::ComponentModifier, cell::Cell{:R},
    cells::Vector{Cell}, proj::Project{<:Any})
...
function cell_highlight!(c::Connection, cm::ComponentModifier, cell::Cell{:R},
    cells::Vector{Cell}, proj::Project{<:Any})
...
function r_block!(tm::ToolipsMarkdown.TextStyleModifier)
    ToolipsMarkdown.mark_between!(tm, "\"\"\"", :multistring, exclude = "\"\"\"")
    ToolipsMarkdown.mark_between!(tm, "\"", :string)
    ToolipsMarkdown.mark_all!(tm, "function", :func)
...
function highlight_r!(tm::ToolipsMarkdown.TextStyleModifier)
...
function read_r(uri::String)
    pyd = split(read(uri, String), "\n\n")
    [Cell(e, "R", string(line)) for (e, line) in enumerate(pyd)]
end

function build(c::Connection, cell::Cell{:r},
    d::Directory{<:Any}; explorer::Bool = false)
    filecell = Olive.build_base_cell(c, cell, d, explorer = explorer)
    style!(filecell, "background-color" => "darkblue", "cursor" => "pointer")
    filecell
end

function r_string(c::Cell{<:Any})

end

function r_string(c::Cell{:R})
    c.source
end

function string(c::Cell{:R})
    """R\"\"\"$(c.source)\"\"\"
    #==output[python]
    $(c.outputs)
    ==#
